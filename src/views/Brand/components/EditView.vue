<template>
    <el-tooltip content="Cancel modification, return to the list.">
    ...
    <el-form
        ...
        >
        <h1 class="content-title-class" tag="b">Modify brand</h1>
...
const ruleForm = reactive<Data>({
    ...
    brandName: (prop.row as Data).brandName,
    type: (prop.row as Data).type,
...
const handleRemove = (file: any, fileL: { [x: string]: unknown; brandId?: number | undefined; brandName?: string | undefined; image?: string | undefined; type?: number | undefined; file?: string | { readonly size: number; readonly type: string; arrayBuffer: () => Promise<ArrayBuffer>; slice: (start?: number | undefined, end?: number | undefined, contentType?: string | undefined) => Blob; stream: () => ReadableStream<Uint8Array>; text: () => Promise<string>; } | undefined; }[]) => {
...
[x: string]: unknown;
// import OperateButton from '@/components/OperateButton.vue'; 
brandId?: number | undefined; brandName?: string | undefined; image?: string | undefined; type?: number | undefined; file?: string | { readonly size: number; readonly type: string; arrayBuffer: () => Promise<ArrayBuffer>; slice: (start?: number | undefined, end?: number | undefined, contentType?: string | undefined) => Blob; stream: () => ReadableStream<Uint8Array>; text: () => Promise<string>; } | undefined;
}[]) => {
...
const checkUploadTx = (file: { name: string; size: number; }) => {
    ...
    if (!isJPG) {
        ElMessage.warning(`Profile picture can only be ${imgTypeA.join('、')} format!`);
    }
    if (!isLt2M) {
  ...
  formEl.validate(async(valid) => {
    if (valid) {
        ...
        if(hasFile.value || fileList.value.length > 0){
            const res = await brandsStore.updateDataAction(ruleForm)
            if(res){
